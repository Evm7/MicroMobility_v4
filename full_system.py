...
from Extras.TrafficLight import TrafficLight
from Extras.Collisions import Collisions
...
class FullSystem():
    ...
    def __init__(self):
        ...
        self.total_time_trafficLight = 0  # to measure the total amount of time for traffic light color detection
        self.total_time_application = 0  # to measure the total amount of time for writting application text
        self.total_time_plotting = 0  # to measure the total amount of time for plotting video map distance
        self.total_time_collisions = 0  # to measure the total amount of time for collision alarm
        ...
        (self.input_path, self.output_path, self.resultsFile, self.visualize, self.maxDisappeared, self.maxDistance,
         self.skipFrames, self.scale, self.rotate, self.directory, self.distance, self.application, self.plotting, self.collisions) = self.argument_parsing()
        ...
        if self.collisions == True and self.distance == False:
            print("We can not provide a collission alarm if 'Distance computation' is not enabled: set '--distance True'")
        self.collisions = self.collisions and self.distance
    ...
    def argument_parsing(self):
        ...
        parser.add_argument('--application', type=bool, default=False,
                            help='Introduce True if wants to write down the android app detection report')
        parser.add_argument('--plotting', type=bool, default=False,
                            help='Introduce True if wants to plot the distance in real time')
        parser.add_argument('--collisions', type=bool, default=False,
                            help='Introduce True if wants to alarm with the collisions probability')

        args = parser.parse_args()
        return args.i, args.output, args.resultsFile, args.visualize, args.maxDisappeared, args.maxDistance, args.skipFrames, args.scale, args.rotate, args.directory, args.distance, args.application, args.plotting, args.collisions
    ...
    def write_video(self, video_in, output_path, scale):
        ...
        size = (int(video_in.get(cv2.CAP_PROP_FRAME_WIDTH) / scale), int(video_in.get(cv2.CAP_PROP_FRAME_HEIGHT) / scale))
    ...
    def write_application_json(self, directory):
        """
        Enables writing down results of the detected video used afterwards for the android application
        :param directory: name of the directory where document will be placed in
        :return:
        """
        write = self.getInformation()
        try:
            f = open('out/' + directory + '/' + "Manual_detections.json", "x+")
            f.write(write)
        except Exception as e:
            print("There was an error while creating file to write application file detections")
            print(str(e))

    def write_collisions(self, directory, info):
        """
        Enables writing down collisions results
        :param directory: name of the directory where document will be placed in
        :param info: information of the collisions to write
        :return:
        """
        try:
            f = open('out/' + directory + '/' + "Collisions.txt", "x+")
            f.write(info)
        except Exception as e:
            print("There was an error while creating file to write application file detections")
            print(str(e))
    ...
    def getInformation(self):
        """
        Enables writing down configurations and results of the detected video
        :return: the information of all detections in json format
        """
        map = self.createDetectionsMap()
        self.adaptMapToAndroid(map)
        self.detections["Type of Road"] = [] # We do not have incorporated the AI road detection in this script yet
        return json.dumps(self.detections, indent=5)


    def createDetectionsMap(self):
        total = {}
        for tobj in self.trackableObjects.values():
            cat = tobj.category.split(':')[0]
            start_time = tobj.startFrame * self.fps
            end_time = tobj.endFrame * self.fps

            if cat in total:
                if start_time in total[cat]:
                    value_start = total[cat][start_time] + 1
                else:
                    value_start = 1

                if end_time in total[cat]:
                    value_end = total[cat][end_time] - 1
                else:
                    value_end = -1
            ...
            total[cat][start_time] = value_start
            total[cat][end_time] = value_end

        return total

    def adaptMapToAndroid(self, map):
        """
        Introduces the started millisecond of the object and computes the counting
        :param map: Map of the whole detections made
        :return: we add the result to self.detections
        """
        for cat, mapper in map.items():
            if cat not in self.detections:
                self.detections[cat] = []
            for time in sorted(mapper.keys()):
                new_obj = {}
                new_obj["Time"] = time
                if len(self.detections[cat])>0:
                    value = self.detections[cat][-1]['Value']+ mapper[time]
                else:
                    value = mapper[time]
                new_obj["Value"] = value
                self.detections[cat].append(new_obj)
    ...
    def write_initial_config(self, directory):
        ...
        configurations += "\t- Application : " + str(self.application) + "\n"
        configurations += "\t- Plotting : " + str(self.plotting) + "\n"
        configurations += "\t- Collision Alarm : " + str(self.collisions) + "\n"
    ...
    def write_final_config(self, configurations):
        ...
        """
        total_time = self.total_time_application + self.total_time_plotting + self.total_time_collisions + self.total_time_distance_computing + self.total_time_lane_detection + self.total_time_trafficLight + self.total_time_tracking + self.total_time_detection + self.total_time_centroid
        ...
        final_str += "Total amount of time for whole video traffic light color detection " + "%0.4f" % self.total_time_trafficLight + ' seconds: ' + "%0.4f" % (
                    self.total_time_trafficLight / total_time) + "\n"
        ...
        final_str += "Total amount of time for writting appication file " + "%0.4f" % self.total_time_application + ' seconds: ' + "%0.4f" % (
                    self.total_time_application / total_time) + "\n"
        ...
        final_str += "Total amount of time for whole video collision detection alert " + "%0.4f" % self.total_time_collisions + ' seconds: ' + "%0.4f" % (
                    self.total_time_collisions / total_time) + "\n"
        final_str += "____________________________________________________________" + "\n"
        final_str += "Trackable objects: \n" + str(self.trackableObjects).replace(',', '\t').replace('{',
                                                                                                     '\t').replace('}',
                                                                                                                   '')
    ...
    def processVideo(self):
        ...
        if self.output_path:
            ...
            (out, length, self.fps, self.size) = self.write_video(self.cap, directory + "/" + self.output_path, self.scale)
        else:
        ...
        if self.collisions:
            collision = Collisions(self.size)
            str_collissions = "COLLISIONS INFORMATION FOR THE VIDEO:\n"
            ...
            if r:
                ...
                if frame_id % self.skipFrames == 0:
                    ...
                    for cat, score, bounds in results:
                        ...
                        if name in self.class_names:
                            ...
                            ### print("Category "+ name+ ": dimensions { " + "left "+str(left) + ", right "+ str(right) +", bottom "+ str(bottom) +", top "+ str(top) + " }", flush=True)
                ...
                if self.distance:
                    ...
                    if lines is not None and len(lines) >0:
                        ...
                        start_time = time.time()
                        if self.plotting:
                            distances, coord = self.distance_calculator.computeAllDistances(none_original, lines, bboxes)
                        else:
                ...
                for (objectID, centroid) in objects.items():
                    ...
                    if name != "traffic light":
                        ...
                        color_light = traffic_light.light_detection(bboxes[objectID], reformat_frame)
                        if color_light == "None" or color_light == "Error":
                            cv2.putText(reformat_frame, str(categ[objectID]),
                                        (bboxes[objectID][2], bboxes[objectID][3]), cv2.FONT_HERSHEY_COMPLEX, 1,
                                        self.dictionary[name])
                        else:
                            cv2.putText(reformat_frame, str(categ[objectID]) + "-->" + color_light,
                                        (bboxes[objectID][2], bboxes[objectID][3]), cv2.FONT_HERSHEY_COMPLEX, 1,
                                        self.dictionary[name])
                        tobj.setOption("traffic light", color_light)
                    self.trackableObjects[objectID] = tobj
                    self.total_time_trafficLight += time.time() - start_time
                        ...
                        if self.visualize:
                            if objectID in distances.keys():
                                ...
                                if distances[objectID] < 2:
                                    print("Proximity alert: "+ name+ " "+ str(objectID)+ " is located at " + str(distances[objectID]) +" meters")
                ...
                if distances is not None:
                    ...
                    if self.collisions:
                        start_time = time.time()
                        probabilities = collision.checkAllCollisions(current_velocity, velocities, bboxes, categ)
                        a = 0
                        for id, prob in probabilities.items():
                            if prob[1] is not None:
                                info = "Object " + str(categ[id]) + " at times " + str(
                                    prob[1]) + " has a collission probability of " + str(
                                    [str("{:.2f}".format(i)) for i in prob[0]])
                                cv2.putText(reformat_frame, info, (80, 40 + 40 * a), cv2.FONT_HERSHEY_COMPLEX, 0.5,
                                            (0, 0, 255))
                                cv2.circle(reformat_frame, (40, 40 + 40 * a), 20, (0, 0, 255), 3)
                                a += 1
                                str_collissions += info + "\n"
                        self.total_time_collisions += time.time() - start_time
                ...
                if self.output_path:
                    ...
                    if (percentage == 100):
                        ...
                        if self.application:
                            start_time = time.time()
                            self.write_application_json(directory)
                            self.total_time_application += time.time() - start_time

                        if self.collisions:
                            start_time = time.time()
                            self.write_collisions(directory, str_collissions)
                            self.total_time_collisions += time.time() - start_time
